...
| **Overwrite another citizen's profile** (`POST /register`) by sending their known `sender_id` | **None.** `sender_id` is a device's Ed25519 public key, which travels in the clear in every mesh packet (T2) and in `GET /responders/keys` for responders — it is not secret. `/register` upserts on `sender_id` with **no proof the caller holds the matching private key**. | New: every field bounded (was unbounded free text — see `schemas/user_profile.py`), closing the storage/DoS angle | **Real, unfixed spoofing/tampering risk**: anyone who has observed a target's `sender_id` can overwrite their name, age, gender, `medical_history`, and `emergency_contacts`. A real fix (require the request to be signed, verified via the existing `signature_service.py`) is architecturally sound and `/register` is explicitly online-only (no offline-ingestion constraint like `/ingest`), but it requires a coordinated change to `setu_app`'s `ProfileSyncService` to actually sign the payload — not shipped this sprint as an untested breaking change to a working flow. **Second-highest-priority follow-up**, after T7. |
